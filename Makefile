...
HTTPS_GIT := https://github.com/MonikaCat/canine-chain/v2.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=canine \
		  ...
		  -X github.com/MonikaCat/canine-chain/v2/app.Bech32Prefix=jkl \
...
test-sim-bench:
	@VERSION=$(VERSION) go test -benchmem -run ^BenchmarkFullAppSimulation -bench ^BenchmarkFullAppSimulation -cpuprofile cpu.out github.com/MonikaCat/canine-chain/v2/app
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/MonikaCat/canine-chain/v2
